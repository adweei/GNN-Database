...
class GATv2_model_run():
    ...
    def draw_GAT_Loss(self, PATH, loss_list, acc_list):
        for graph_num in range(len(os.listdir(PATH)) + 1):
            ...
            if not os.path.isfile(PATH + str(graph_num) + '_GATv2_Loss0930.png'):    
                ...
                plt.savefig(PATH + str(graph_num) + '_GATv2_Loss0930.png')
                plt.close()

    def GATv2_run_model(self):
        ...
        gatv2conv = GATv2Conv(8, 2, num_heads = 2)
        res = gatv2conv(g, self.features.size())
