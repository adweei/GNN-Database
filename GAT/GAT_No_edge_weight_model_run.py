...
class GAT_model_run():
    ...
    def draw_GAT_Loss(self, PATH, loss_list, acc_list):
        for graph_num in range(len(os.listdir(PATH)) + 1):
            ...
            if not os.path.isfile(PATH + str(graph_num) + '_GAT_Loss1001.png'):    
                ...
                plt.savefig(PATH + str(graph_num) + '_GAT_Loss1001.png')
                plt.close()

    def GAT_run_No_edge_weight_model(self):
        # print(self.features.size()[1])   #feature = 200 + 4
        print('GAT(No edge weight): ', self.graph)
        self.net = GAT( 
        g = self.graph,
        ...
        for epoch in range(self.Epoch_GAT):
            ...
            # gat_loss = focal_loss(gat_logits[self.train_mask], self.labels[self.train_mask])

            # print(gat_logits.size(), self.labels.size())
            # gat_loss = FL.sigmoid_focal_loss(gat_logits[self.train_mask], self.labels[self.train_mask])
            ...
            print("GAT(No edge weight): Epoch {:05d} | Loss {:.4f} | Test Acc {:.4f} | Time(s) {:.4f}".format(
