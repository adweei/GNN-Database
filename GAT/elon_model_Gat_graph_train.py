...
from GAT_NoEdgeWeight import GAT
...
def retweet_count(g, features, labels, retweet_distribution_mask, retweet_count_model_list):
    ...
    for seq in range(3):
        ...
        net.load_state_dict(th.load(model_choose_file_dir + str(retweet_count_model_list[seq][1]) +
                                    '/' + str(retweet_count_model_list[seq][2]) + '_round_result_edge_weight_Adjustment_GAT.pth'))
        ...
        if seq == 0:
            retweet_count_model.model_save(data_graph_dir + str(number[0]) + "/" + str(time_point) + "/" + str(number[0]) +
                                          '_high_retweet_count_GATmodel_result')
            print('----------------------------------------------------------high retweet count---------------------------------------------------------------------')  
        elif seq == 1:
            retweet_count_model.model_save(data_graph_dir + str(number[0]) + "/" + str(time_point) + "/" + str(number[0]) +
                                          '_middle_retweet_count_GATmodel_result')
            print('-------------------------------------------------------second high retweet count-----------------------------------------------------------------')            
        elif seq == 2:
            retweet_count_model.model_save(data_graph_dir + str(number[0]) + "/" + str(time_point) + "/" + str(number[0]) +
                                          '_low_retweet_count_GATmodel_result')
...
def retweet_time(g, features, labels, retweet_distribution_mask, distribution_model_list):
    for time in range(6):
        ...
        net.load_state_dict(th.load(model_choose_file_dir + str(distribution_model_list[time][1]) + '/'+ 
                                    str(distribution_model_list[time][2]) + '_round_result_edge_weight_Adjustment_GAT.pth'))
        ...
        if time == 0:
            retweet_time_model.model_save(data_graph_dir + str(number[0]) + "/" + 
                                        str(time_point) + "/" + str(number[0]) + '_over_84_GATmodel_result')
            print('----------------------------------------------------------over 84H---------------------------------------------------------------------')
        elif time == 1:
            retweet_time_model.model_save(data_graph_dir + str(number[0]) + "/" + 
                                      str(time_point) + "/" + str(number[0]) + '_between_60_83_GATmodel_result')
            print('---------------------------------------------------between 60H to 83H------------------------------------------------------------------')
        elif time == 2:
            retweet_time_model.model_save(data_graph_dir + str(number[0]) + "/" + 
                                      str(time_point) + "/" + str(number[0]) + '_between_50_59_GATmodel_result')
            print('---------------------------------------------------between 50H to 59H------------------------------------------------------------------')
        elif time == 3:
            retweet_time_model.model_save(data_graph_dir + str(number[0]) + "/" + 
                                      str(time_point) + "/" + str(number[0]) + '_between_31_49_GATmodel_result')
            print('---------------------------------------------------between 31H to 49H------------------------------------------------------------------')
        elif time == 4:
            retweet_time_model.model_save(data_graph_dir + str(number[0]) + "/" + 
                                      str(time_point) + "/" + str(number[0]) + '_between_25_30_GATmodel_result')  
            print('---------------------------------------------------between 25H to 30H------------------------------------------------------------------')
        elif time == 5:
            retweet_time_model.model_save(data_graph_dir + str(number[0]) + "/" + 
                                      str(time_point) + "/" + str(number[0]) + '_in_12_GATmodel_result')    
            print('---------------------------------------------------under 24H---------------------------------------------------------------------------')          
        del net        

#-------------------------------------------將data graph的tag資訊一起讀出----------------------------------------------------------------
data_graph_dir = elon_mask_test_graph_data_dir + 'no_weight_data_graph/'
tag_dir =  elon_mask_test_graph_data_dir + 'data_graph/'
data_graph = open(tag_dir + 'test_graph_tag.txt')           #神秘tag咒語，請手建
...
#-------------------------------------------將tag要選擇的model給讀取出來--------------------------------------------------------------------
model_choose_file_dir = elon_mask_base_graph_data_dir + 'base_graph_for_model/GAT_graph_no_weight_model/'
model_choose_dir = elon_mask_base_graph_data_dir + 'base_graph_for_model/'
tag_model = open(model_choose_dir + 'GAT_model_for_feature(duration).txt')
tag_model_list = list()
...
retweet_count_model = open(model_choose_dir + 'GAT_model_for_retweet.txt')
...
# #-----------------------------------------將不同時間點要選的model讀取出來----------------------------------------------------------------
distribution_model = open(model_choose_dir + 'GAT_model_for_distribution.txt')
...
fixed_distribution_model = open(model_choose_dir + 'GAT_fixed_feature_model_for_distribution.txt')
...
#----------------------------------------------不同test graph挑不同feature-----------------------------------------------------------------------
        ...
        if int(number[1]) == 1:
            ...
            #                             str(temp_tag_model_list[np.where(temp_tag_model_list[:,1] == (number[2]))][0][3]) +
            #                             '_round_result_edge_weight_Adjustment_GAT.pth'))
            # tag_test = model_test()
            # print('tag\'s GAT(post) acc is: {:.6f}'.format(tag_test.test_model(net, cuda_g, features, labels, retweet_distribution_mask)))
            # tag_test.model_save(data_graph_dir + str(number[0]) + "/" +
            #                     str(time_point) + "/" + str(number[0]) + '_tag_GATmodel_result')
            # del net
            ...
            # fixed_fearure_retweet_time(cuda_g, features, labels, retweet_distribution_mask, fixed_distribution_model_list)
            ...
            net.load_state_dict(th.load(model_choose_file_dir + str(temp_tag_model_list[np.where(temp_tag_model_list[:,1] == (number[2]))][0][2]) + '/' +
                                        ...
                                        '_round_result_edge_weight_Adjustment_GAT.pth'))
            tag_test = model_test()
            print('tag\'s GAT(reply someone) acc is: {:.6f}'.format(tag_test.test_model(net, cuda_g, features, labels, retweet_distribution_mask)))
            tag_test.model_save(data_graph_dir + str(number[0]) + "/" +
                                str(time_point) + "/" + str(number[0]) + '_tag_GATmodel_result')
            del net
        ...
        #     temp_tag_model_list = tag_model_list[np.where(tag_model_list[:,0] == (number[1]))]
            ...
            #                             '_round_result_edge_weight_Adjustment_GAT.pth'))
            # tag_test = model_test()
            # print('tag\'s GAT(reply own) acc is: {:.6f}'.format(tag_test.test_model(net, cuda_g, features, labels, retweet_distribution_mask)))
            # tag_test.model_save(data_graph_dir + str(number[0]) + "/" +
            #                     str(time_point) + "/" + str(number[0]) + '_tag_GATmodel_result')
            # del net
